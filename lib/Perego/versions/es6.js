...
const Menu = (color) => {//color of menu
	...
	if (!nav.validate.main) {
		...
		const hide = document.createElement("script");
...
const Option = (text, link, location, color) => {//href, innerHTML, float, color(if different from inherited)
...
const nav = {
	...
	set css(rule) {//What is the css to add 
		...
		add.id = "navBarCss" + this.counter.cssn;
		add.appendChild(document.createTextNode(""));//Webkit Hack
		document.head.appendChild(add);
		const node = add.sheet;
	...
	menu: function(menu, main) {//the menu object, is it the main menu and if not what is the id of the option it's in
		...
		for (let i = 0; i < Object.keys(menu.optionList).length; i++) {
	...
	option: function(option, menu) {
		...
		const addShell = document.createElement("li");
		...
		if (/^function:/.test(option.link)){//There must be a better way to do this//This must be AFTER I create the hide button
			...
			const click = document.createElement("script");
