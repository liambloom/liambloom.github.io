...
//Embed with <script src="https://liambloom.github.io/lib/PeregoBeta/PeregoBetaBeta.js"></script>
//jshint multistr: true, esversion: 5
function MenuBeta(options, color) {//color of menu
	console.log("hi")
	Object.defineProperties(this, {
		id: {
			value: "bar" + peregoBeta.counter.bar
		},
		color: {
			value: peregoBeta.validate.color(color)
		},
		optionList: {
			value: {},
			enumerable: true,
		...
		option: {
			set: function(option) {
				if (typeof option === "object") {
					if (option.constructor === OptionBeta) {
						option.parent = this;
						if (!/^!/.test(option.link) && typeof this.parent !== "undefined") {
							option.link = option.link.replace(/^/, this.parent.link.substring(0, this.parent.link.lastIndexOf("/")));
						} 
						else {
							option.link = option.link.replace(/^!/, "");
						}
						this.optionList[peregoBeta.validate.name(option.text)] = option;
					}
					else if (option[Object.keys(option)[0]].constructor === OptionBeta) {
						for (var i in option) {
							//if (i !== "all") {
	...
	if (!peregoBeta.validate.main) {
		peregoBeta.css = peregoBeta.css;
		...
		Object.defineProperties(this, {
			hide: {
				set: function(button) {
					if (button.constructor === OptionBeta){
	...
	if (Array.isArray(options)) {
		...
		if (Array.isArray(options[0])) {
			...
			for (var i in options) {
				this.option = new OptionBeta(options[i][0], options[i][1], options[i][2], options[i][3]);
			...
			this.option = new OptionBeta(options[0], options[1], options[2], options[3]);
		}
	}
	/*else {
...
function OptionBeta(text, link, location, /*super, sub, */color) {//href, innerHTML, float, color(if different from inherited)
	...
	Object.defineProperties(this, {
		...
		location: {
			value: peregoBeta.validate.location(location)
		},
		color: {
			value: peregoBeta.validate.color(color)
		},
...
var peregoBeta = {
	...
	menu: function(menu, main) {//the menu object, is it the main menu and if not what is the id of the option it's in
		try {
			if (typeof menu !== "object" || menu.constructor !== MenuBeta) {
				throw "up";// :P
			}
		}
		catch(err) {
		...
		add.id = peregoBeta.validate.name(menu.id);
		...
		for (var i in menu.optionList) {
			//if (i !== "all") {
				peregoBeta.option(menu.optionList[i], menu.id);
			//}
		}
		//console.log("Ran peregoBeta.menu()")
	},
	option: function(option, menu) {
		try {
			if (typeof option !== "object" || option.constructor !== OptionBeta) {
				throw "up";// :P
			}
		}
		catch(err) {
		...
		addShell.id = "shell" + peregoBeta.validate.name(option.text);
		addShell.className = option.location;
		document.getElementById(menu).appendChild(addShell);
		var add = document.createElement("a");
		add.id = peregoBeta.validate.name(option.text);
		...
		if (typeof option.innerMenu === "object") {
			peregoBeta.menu(option.innerMenu, addShell.id);
		}
		//console.log("Ran peregoBeta.option()")
	}
};