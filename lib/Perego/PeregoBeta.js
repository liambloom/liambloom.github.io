...
var nav = {
	...
	validate: {
		...
		get main() {
			for (var i = 0/*, found = false*/; i < Object.keys(nav.bars).length/* && !found*/; i++) {
				if (/^main$/i.test(Object.keys(nav.bars)[i].level)) {
	...
	menu: function(id, orinetation, level, color, hidebutton) {//id of menu, orinetation of menu, main/sub, menu color, hide button object
	...
	option: function(link, text, menu, location, color) {//href, innerHTML, what menu it's in, float, color(if different from inherited)
		if (this.validate.init) {
			...
			nav.bars[menu].options[text.replace(/\s/gi, "_").replace(/^\d/i, "$").replace(/\W/gi, "$")] = {
				...
				color: color,
				innerMenu: {
					add: function(menu) {//option to put menu in, menu option is in, menu to put in option
						//if (this.validate.init) {//unnececary 
	...
	get end() {
		...
		try {*/
			...
			if (mainMenu.color !== undefined) {
				addMenu.style = "color:" + mainMenu.color + ";";
			}
