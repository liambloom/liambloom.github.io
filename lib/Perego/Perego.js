...
var nav = {
	...
	menu: function(id, orientation, level, color) {//id of menu, orientation of menu, main/sub, menu color, hide button object
		if (this.validate.init) {
			if (typeof id !== "string") {id = "navBar" + this.counter.bar;}
			if (typeof orientation !== "string") {orientation = "horizontal";} else if (!/^(vertical|horizontal)$/i.test(orientation)) {orientation = "horizontal";}//See note below
			...
			/*if (level === "main") {
				if (typeof hidebutton !== "object" && typeof hidebutton !== null) {if (orientation === "horizontal") {hidebutton = new this.hidebutton(undefined, "right");} else {hidebutton = new this.hidebutton();}}
			}
			else {
				hidebutton = null;
			}*/
			nav.bars[id] = {
				...
				color: color,
				options: {},
				set hideButton(button) {//Make this only avalable for the main menu
					if (this.level === "main") {
	...
	option: function(link, text, menu, location, color) {//href, innerHTML, id of menu it's in, float, color(if different from inherited)
		if (this.validate.init) {
			...
			location = this.validate.location(location, nav.bars[menu].orientation);
