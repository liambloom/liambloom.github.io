...
var $ = {
	...
	browser: {//Supports all browsers that have at least 0.3% of users according to wikipedia as of 1/12/19 as well as chromium
		get which() {
			//Chrome, Firefox, Safari, Edge, IE, IE11, Opera, UC, Chromium, Samsung Internet, Android Browser
			var ua = window.navigator.userAgent;
			if (/(?=.*Chrome)(?!(.*Chromium|.*OPR|.*SamsungBrowser|.*Edge|.*YaBrowser))/im.test(ua)) {//Chrome
				return "Chrome";
			}	
			else if (/(?=.*Safari)(?=(.*iPhone|.*iPad|.*Mac))(?!(.*Chromium|.*OPR|.*Edge|.*UCBrowser))/im.test(ua)) {//Safari !UNTESTED
				return "Safari";
			}
			...
			else if (/YaBrowser/im.test(ua)) {//Yandex
				return "Yandex";
			}
			else if (/Chromium/im.test(ua)) {//Chromium
				return "Chromium";
			}
			else {
				return "Other";
