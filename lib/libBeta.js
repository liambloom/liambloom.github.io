...
var $ = {
	...
	browser: {
		get which() {
			...
			else if (/(?=.*Safari)(?=(.*iPhone|.*iPad|.*Mac))(?!(.*OPR|.*SamsungBrowser|.*Edge|.*UCBrowser))/im.test(ua)) {//Safari !UNTESTED
				return "Safari"
			}
			...
			else if (/YaBrowser/im.test(ua)) {//Yandex Browser
				return "Yandex"
			}
			else {
				return undefined
