...
var $ = {
	...
	browser: {
		get which() {
			...
			if (/(?=.*Chrome)(?=.*Safari)(?=.*((?!(OPR|SamsungBrowser)).))/im.test(ua)) {//Chrome !returns true for opera, samsung browser, and edge
				return "Chrome"
			}
			else if (/Safari/im.test(ua)) {//Safari !UNTESTED
				return "Safari"
			}
			else if (/Firefox/im.test(ua)) {//Firefox
				return "Firefox"
			}
			else if (/(?=.*UCBrowser)(?=.*U2)/im.test(ua)) {//UC !UNTESTED
				return "UCBrowser"
			}
			else if (/Opera|OPR/im.test(ua)) {//Opera !UNTESTED Instead of "OPR" I had "(?=.*Chrome)(?=.*Safari)(?=.*OPR)"
				return "Opera"
			}
			else if (/((?=.*Trident)(?=.*((?!MSIE).)))|MSIE/im.test(ua)) {//IE
				return "IE"
			}
			else if (/SamsungBrowser/i.test(ui)) {//Samsung Browser
