...
var $ = {
	...
	browser: {
		get which() {
			...
			else if (/(?=.*Safari)(?!(.*OPR|.*SamsungBrowser|.*Edge))/im.test(ua)) {//Safari !UNTESTED
				return "Safari"
			}
			...
			else if (/SamsungBrowser/im.test(ui)) {//Samsung Browser
				return "Samsung Internet"
			}
			else if (/Edge/im.test(ui)) {//Edge
				return "Edge"
			}
			else {
				return undefined
