	...
	browser: {//Supports Chrome/Dolphin, Firefox, UC, Opera, IE, Samsung, Edge, ASOP Browser, Yandex, Chromium, Blackberry Browser
		...
		get engine() {
			if (/AppleWebKit/im.test(this.ua)) {//Most Browsers
				...
				return /Gecko/i;//this is true when the string contains "like Gecko", which most non-gecko engines do
			}
		...
		is: function(target) {
			if (this.which.test(target) || this.engine.test(target)) {
				return true;
			}
			else {
				return false;
