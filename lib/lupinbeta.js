...
$.key.list[44] = /PrtSc|(Print Screen)/i;//Print Screen
$.key.list[45] = /insert/i;
$.key.list[46] = /delete/i;
$.key.list[47] = /help/i;
for (var i = 48; i <= 57; i++) {//numbers
	$.key.list[i] = new RegExp((i - 48).toString(), "i");
}
$.key.list[58] = /colin|:/i;
...
//$.key.list[91] = /(left (window|⌘|cmnd|command|(mac cmnd)|(mac command)))|Chromebook Search Key/i;
if ($.browser.is("Windows")) {
	$.key.list[91] = /left (window|(window key)|(windows key))/i;//There's probably a better way to do that
}
else if ($.browser.is("MacOS")) {
	$.key.list[91] = /left (⌘|cmnd|command|(cmnd key)|(command key))/i;//There's probably a better way to do that too
}
else if ($.browser.is("ChromeOS")) {
	$.key.list[91] = /search/i;
}
if ($.browser.is("Windows")) {
	$.key.list[92] = /right (window|(window key)|(windows key))/i;//There's probably a better way to do too
}
if ($.browser.is("Windows")) {
	$.key.list[93] = /select|menu/i;
}
else if ($.browser.is("MacOS")) {
	$.key.list[93] = /select|right (⌘|cmnd|command|(cmnd key)|(command key))/i;//There's probably a better way to do that too
}
$.key.list[95] = /sleep/i;
for (var i = 96; i <= 105; i++){//numpad
	$.key.list[i] = new RegExp("numpad " + (i - 96).toString(), "i");
}
...
$.key.list[169] = /(closing paren)|AZERTY/i;//I don't know what this is
$.key.list[170] = /star|asterisk|\*/i;
$.key.list[171] = /~|\+|\*/i;//wait, what?
$.key.list[172] = /home/i;
if (!$.browser.is("Firefox")) {
	$.key.list[173] = /subract|minus|-/i;
}
else {
	$.key.list[173] = /mute|unmute/i;//There is probably a better way to do this
}
...
$.key.list[193] = /\?|,|\/|°/i;//That seems wrong
...
$.key.list[225] = /altgr/i;//What?
$.key.list[226] = /(< \/git>)|(left (backslash|back slash|\\))/
$.key.list[230] = /GNOME Compose Key/i;//What?
$.key.list[231] = /ç/i;
$.key.list[233] = /XF86Forward/i;//What?
$.key.list[234] = /XF86Back/i;//What?
$.key.list[235] = /non-conversion/i;//Wasn't this earlier
$.key.list[240] = /alphanumeric/i;//What?
$.key.list[242] = /hiragana|katakana/i;//What?
$.key.list[243] = /half-width|full-width/i;//What?
$.key.list[244] = /kanji/i;//What?
